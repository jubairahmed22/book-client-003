    ...
    const fetchCategory = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:5001/api/admin/bannerTwo/${categoryId}`
        );
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.put(
        `http://localhost:5001/api/admin/update/bannerTwo/${categoryId}`,
        formDataToSend,
        {
          headers: {
      ...
      Swal.fire("Success!", "Category updated successfully!", "success").then(
        () => {
          router.push("/admin/banner/banner-two");
        }
      );
    } catch (err) {
      setError(err.message);
...
export default EditBannerTwoPage;
